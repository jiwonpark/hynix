...
class StrategyLabTests(unittest.TestCase):
    def test_dual_stack_enters_at_next_five_minute_open(self):
        five_prices = list(range(100, 180))
        hour_prices = list(range(100, 180))
        ...
        signal_index = next(i for i, row in enumerate(result["bars"]) if row["bullish"])
    ...
    def test_hourly_stack_is_required_when_enabled(self):
        five = candles(0, 80, 300, list(range(100, 180)))
        falling_hours = candles(-60 * 3600, 80, 3600, list(range(180, 100, -1)))
        blocked = run_ma_stack_backtest(five, falling_hours, entry_1h=True)
        allowed = run_ma_stack_backtest(five, falling_hours, entry_1h=False)
