...
(async () => {
  ...
  assert.equal(elements.get('lab_btnModeLive').disabled, false);
  assert.match(elements.get('lab_btnModeLive').title, /Click to unlock/);
  assert.equal(elements.get('lab_inpBotTrancheSize').disabled, true);
